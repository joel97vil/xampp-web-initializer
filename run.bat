...
set url="example.com"
set port="80"
set hostsFile="C:\Windows\System32\drivers\etc\hosts"   #Default path for Windows
set vhostsFile="C:\xampp\apache\conf\extra\httpd-vhosts.conf"   #Default path for Windows
...
:loop
    IF %param% == "i" set indexPath=%param% #INDEX.PHP OR INDEX.HTML PATH (REQUIRED)
    IF %param% == "p" set vhosts_file=%param% #VHOST PORT (NOT-REQUIRED, DEFAULT 80)
    ::IF %param% == "v" set vhosts_file=%param% #VHOST CONFIG PATH (NOT-REQUIRED, DEFAULT WINDOWS)
    ::IF %param% == "h" set hosts_file=%param% #HOSTS RESOLVE PATH (NOT-REQUIRED, DEFAULT WINDOWS)
